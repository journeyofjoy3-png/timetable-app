...
if selected == "Timetable Analyzer":
    ...
    with st.expander("📁 Upload / Update Timetables", expanded=True):
        ...
        if mode == "Class Timetable Only":
            ...
            with upload_col1: class_file = st.file_uploader("1. Upload Class Timetable (Excel)", type=["xlsx", "xls"], key="class_file")
            with upload_col2: doubt_file = st.file_uploader("2. Upload Doubt Timetable (Excel)", type=["xlsx", "xls"], key="doubt_file")
    ...
    if class_file is not None:
        if mode == "Class + Doubt Timetables" and doubt_file is None: st.warning("Please upload the Doubt Timetable to proceed.")
        else:
...
# ==========================================
    ...
    with st.expander("⚙️ Upload & Config", expanded=True):
        generator_class_file = st.file_uploader("1. Upload Class Timetable (Excel)", type=["xlsx", "xls"], key="gen_class")
                ...
                if not target_teachers:
                    ...
                    for teacher in target_teachers:
                        ...
                        for day, start_col in days_map.items():
                            c0, c1, c2, c3, c4, c5, c6, c7 = [has_class(row, start_col, i) for i in range(8)]
